...
function triggerLayerCallback(layerId) {
    // Show quick touch actions globally after loading screen disappears
    if (layerId !== 'layer-loading') {
        document.getElementById('mobile-touch-controls').classList.remove('hidden');
    }
    ...
    if (layerId === 'layer-hero') {
        ...
        document.getElementById('btn-start').onclick = () => switchLayer('layer-gift');
    }
    ...
    if (layerId === 'layer-gift') {
        ...
        giftBox.onclick = () => {
            if(!giftBox.classList.contains('open')) {
                ...
                document.getElementById('bg-music').play().catch(e => console.log("Audio deferred"));
...
function renderGallery() {
    ...
    APP_STATE.galleryImages.forEach((img, i) => {
        ...
        card.innerHTML = `
            <img src="${img.src}" onerror="this.src='${img.src.replace('images/', '')}'; console.log('GitHub Path Auto-Fixed');" alt="Original Photo">
            <div class="card-caption">${img.caption}</div>
        `;
        slider.appendChild(card);
    });

    // Touch Slide Support for Android Browsers
    let startX = 0;
    slider.ontouchstart = (e) => startX = e.touches[0].clientX;
    slider.ontouchend = (e) => {
        let endX = e.changedTouches[0].clientX;
        if (startX - endX > 50) moveSlider(1); // Swipe Left
        if (endX - startX > 50) moveSlider(-1); // Swipe Right
    };

    document.querySelector('.next-btn').onclick = () => moveSlider(1);
    document.querySelector('.prev-btn').onclick = () => moveSlider(-1);
    document.getElementById('btn-to-letter').onclick = () => switchLayer('layer-letter');
}
...
function setupMobileTouchEggs() {
    ...
    // Android Touch Button 1: Love Rain
    document.getElementById('btn-touch-heart').onclick = (e) => {
        e.stopPropagation();
        APP_STATE.activeEffects = 'hearts';
        showToast("💖 হৃদিপাখির জন্য ভালোবাসার বৃষ্টি শুরু হলো!");
    };

    // Android Touch Button 2: Birthday Rain
    document.getElementById('btn-touch-rain').onclick = (e) => {
        e.stopPropagation();
    ...
    // Moon Touch
    document.getElementById('interactive-moon').onclick = (e) => {
        e.stopPropagation();
    ...
    // Double Tap Cake
    let lastTap = 0;
    document.getElementById('birthday-cake').onclick = () => {
